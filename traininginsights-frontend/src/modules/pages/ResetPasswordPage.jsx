...
export default function ResetPasswordPage(){
  ...
  const submit = async (e) => {
    ...
    try { await api.post('/api/auth/password-reset/confirm', { token, password }); setMsg('Password reset. Redirecting to login...'); setTimeout(()=>navigate('/login'), 1500) } catch { setErr('Reset failed (token invalid or expired)') }
  }
  return (
